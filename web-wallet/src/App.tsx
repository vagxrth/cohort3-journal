...
export default function App() {
  ...
  const generateETHWallet = async ({ seedPhrase }: { seedPhrase: string }) => {
  ...
  const generateSOLWallet = async ({ seedPhrase }: { seedPhrase: string }) => {
    try {
      const seed = await mnemonicToSeed(seedPhrase);
      const path = `m/44'/501'/${currentSOLIndex}'/0'`;
      // control reaches till here
      const derivedSeed = derivePath(path, seed.toString("hex")).key; // this is the line that is probably generating the error
          ...
          <Tabs defaultValue="ethereum" className="w-full">
            ...
            <TabsContent value="ethereum">
              <Card className="border-2 border-black">
                ...
                <CardFooter>
                  <Button onClick={() => generateETHWallet({ seedPhrase: '' })} className="w-full bg-black text-white hover:bg-gray-800">
            ...
            <TabsContent value="solana">
              <Card className="border-2 border-black">
                ...
                <CardFooter>
                  <Button onClick={() => generateSOLWallet({ seedPhrase: '' })} className="w-full bg-black text-white hover:bg-gray-800">
